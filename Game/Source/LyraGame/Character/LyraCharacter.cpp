...
//@TODO: SignificanceManager->RegisterObject(this, (EFortSignificanceType)SignificanceType);
	...
	{
		LyraSettingsShared = GetLyraSettingsShared();
	}
}
...
ULyraSettingsShared* ALyraCharacter::GetLyraSettingsShared() const
{
	const ALyraPlayerController* PC = GetLyraPlayerController();
	if (IsValid(PC))
	{
		const ULyraLocalPlayer* LocalPlayer = Cast<ULyraLocalPlayer>(PC->GetLocalPlayer());
		return LocalPlayer ? LocalPlayer->GetSharedSettings() : nullptr;
	}

	return nullptr;
}

bool ALyraCharacter::GetToggleAim() const
{
	if(LyraSettingsShared == nullptr)
	{
		return GetLyraSettingsShared()->GetToggleAim();
	}
	return LyraSettingsShared->GetToggleAim();
}

bool ALyraCharacter::GetToggleSprint() const
{
	if(LyraSettingsShared == nullptr)
	{
		return GetLyraSettingsShared()->GetToggleSprint();
	}
	return LyraSettingsShared->GetToggleSprint();
}
